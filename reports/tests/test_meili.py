...
class ReportMeiliTest(APITestCase):
    ...
    def setUp(self):
        ...
        # index reports
        call_command('index_reports_meili', '--sync')
        # create test user
    ...
    def test_check_meili(self):
        ...
        self.assertEqual(response.data['count'], 8)
        response = self.client.get('/webapi/v2/browse/reports/', { 'year': '20372512783932' })
        self.assertEqual(response.status_code, 400)
        response = self.client.get('/webapi/v2/browse/reports/', { 'year': 'thisisnotayear' })
        self.assertEqual(response.status_code, 400)
        response = self.client.get('/webapi/v2/browse/reports/', { 'valid_on': '2024-11-11' })
        self.assertEqual(response.data['count'], 9)
        response = self.client.get('/webapi/v2/browse/reports/', { 'valid_on': '2014' })
        self.assertEqual(response.status_code, 400)
        response = self.client.get('/webapi/v2/browse/reports/', { 'valid_on': '20372512783932-10-01' })
        self.assertEqual(response.status_code, 400)
        response = self.client.get('/webapi/v2/browse/reports/', { 'valid_on': 'thisisnoadate' })
        self.assertEqual(response.status_code, 400)
